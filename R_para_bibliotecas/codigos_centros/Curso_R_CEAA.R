...
## CARGAR SET DE DATOS
ceaa = read_csv("archivos_bibliografxs_openrefine/CEAA_limpieza.csv")
...
colnames(cedua) #para saber el nombre de tus columnas, usa la funcion colnames()
colnames(ceaa)
columnas = colnames(ceaa)
...
articulos_nuevas_citas_2017= length(ceaa$`Nuevas 2017`[ceaa$`Nuevas 2017`>0])

#Suma todas las nuevas citas de todos los articulos en 2017
total_nuevas_citas_2017 <- sum(ceaa$`Nuevas 2017`)
...
ceaa$`Validadas 2017`[articulos_citas_validadas_2017<0]
ceaa$`Validadas 2017`[ceaa$`Validadas 2017`>0]
length(ceaa$`Validadas 2017`[ceaa$`Validadas 2017`>0])
Validadas_ceaa-2017=
